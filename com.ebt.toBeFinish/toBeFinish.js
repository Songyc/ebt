	...
	function(viewTemplate, toBeFinish_template, Util, OpenAPI, DateUtil, Locale) {
		return Piece.View.extend({
			...
			delLocaleCourseinfo: function(params) {
				// 删掉待上传列表回显信息
				Util.delLessonsFromCacheByType4Srh(Util.CACHE_TYPE_UNFI_4_DISP, params);
				// 删掉待上传分数信息
				Util.delLessonInfoFromCacheByType(params, Util.CACHE_TYPE_UNFI);
				// 删掉课时列表中的信息
				Util.delLessonInfoFromCacheByType(params, Util.CACHE_TYPE_UNFI_4_DISP);
			},
			goDetial: function($tar) {
				...
				this.navigateModule("com.ebt.course/coursePage?from=toBeFinish", {
					trigger: true
				});
			},
			editCourse: function($tar) {
				...
				this.navigateModule("com.ebt.course/courseStuInfo?comefrom=toBeFinish&courseId=" + courseId + "&lessonId=" + lessonId + "&instructorId=" + instructorId + "&studentId=" + studentId, {
					trigger: true
				});
			},
			...
			queryCourse: function() {
				...
				var queryResult = Util.queryLessonList(queryParams, Util.CACHE_TYPE_UNFI_4_DISP);
			...
			requestData: function() {
				//身份证号
				var that = this;
				var data = Util.getLessonsFromCacheByType4Srh(Util.CACHE_TYPE_UNFI_4_DISP);
